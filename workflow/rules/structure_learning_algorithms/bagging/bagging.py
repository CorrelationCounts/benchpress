...
def alg():
    ...
    # go through the input string and split the ids to make the correct list
    ...
    mats = [
        pd.read_csv(path, header=0).loc[:, lambda df: ~df.columns.str.contains('^Unnamed')]
        for path in adjs
    ]
    ...
    # Save results with proper column names
    colnames = mats[0].columns
    pd.DataFrame(avg_df, columns=colnames).to_csv(out_csv_avg, index=False)
    pd.DataFrame(bin_mat, columns=colnames).to_csv(out_csv_adj, index=False)
